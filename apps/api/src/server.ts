...
app.use(
  cors({
    origin: isProd
      ? [
          env.FRONTEND_URL || "https://chat.cruxsee.in",
...
// Rate limiter: 500 requests per 5 minutes per IP
...
// ─── Health ──────────────────────────────────────────────────────────────
app.get("/", (_req, res) => res.json({ message: "Cruxsee API is up and running" }));
...
// ─── tRPC (kept for backward compat, will phase out) ─────────────────────
app.use(
  "/rest",
