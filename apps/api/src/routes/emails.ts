...
emailsRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    if (label === "INBOX") listParams.q = "in:inbox";
      ...
      listParams.q = "in:trash";
      listParams.includeSpamTrash = true;
    } else if (label === "SPAM") {
      listParams.q = "in:spam";
      listParams.includeSpamTrash = true;
    ...
    // Fetch full details concurrently (max 5 parallel)
    ...
    const result = { emails: detailed, nextPageToken: list.nextPageToken || null };
    await cache.set(cacheKey, result, cache.TTL.emails);

    // Filter out blocked IDs
    if (blockedIds.length > 0) {
      result.emails = result.emails.filter((m) => !blockedIds.includes(m.id));
    }
...
// POST /api/emails/:id/modify — add/remove labels
